...
function initCarousel() {
	jQuery('.gallery, .slideshow').scrollGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
...
function initAddSideClasses() {
	var sideClass = 'right-side';
	var holder = jQuery('#nav');
	var win = jQuery(window);

	holder.find('ul').each(function(i) {
		var drop = jQuery(this);
		var dropHolder = drop.closest('#nav>li');

		drop.css({
			display: 'block'
		});
		dropHolder.removeClass(sideClass);
		if(drop.offset().left + drop.innerWidth() > holder.offset().left + holder.innerWidth()) {
			dropHolder.addClass(sideClass);
		}
		setTimeout(function() {
			drop.css({
				display: ''
...
function initViewportDrop(){
	ResponsiveHelper.addRange({
		'..767': {
			...
			off: function() {
				jQuery('.has-drop-down').each(function() {
